...
export const actions: ActionTree<RootState, RootState> = {
  async fetchModules({ commit }) {
    try {
      const modules = await this.$axios.$get('/admin/modules/')
  ...
  async fetchModule({ commit }, id: string) {
    try {
      const mod = await this.$axios.$get(`/admin/modules/${id}`)
  ...
  async saveModule({ commit }, mod: Partial<Module>) {
    try {
      const { module_id } = await this.$axios.$post('/admin/modules/', mod)
  ...
  async toggleLessonRelease({ state, commit, dispatch }, lessonID: string) {
    try {
      await this.$axios.$post(
        `/admin/modules/${state.currentModule.id}/lesson/${lessonID}/togglerelease`
      )
  ...
  async removeModule({ commit, state }) {
    if (
      ...
      await this.$axios.$delete(`/admin/modules/${state.currentModule.id}`)
  ...
  async updateHeader({ state, commit }) {
    try {
      await this.$axios.$patch(
        `/admin/modules/${state.currentModule.id}/header`,
        {
  ...
  async moveLesson(
    ...
    try {
      await this.$axios.$patch(
        `/admin/modules/${state.currentModule.id}/lesson/${id}/${direction}`
      )
      await dispatch('fetchModule', state.currentModule.id)
    } catch (error) {
      commit('info/SET_ERROR', error, { root: true })
  ...
  async addLesson({ commit, state, dispatch }, id: string) {
    try {
      await this.$axios.$put(
        `/admin/modules/${state.currentModule.id}/lesson/${id}`
      )
      await dispatch('fetchModule', state.currentModule.id)
    } catch (error) {
      commit('info/SET_ERROR', error, { root: true })
